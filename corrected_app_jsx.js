...
// Animations CSS natives remplacent framer-motion
import { 
  uploadFile, 
  classifyImage, 
  exportResults, 
...
// Composant pour les cartes de statistiques avec animation CSS native
...
const ConfidenceLevel = ({ level, label, color, percentage, action }) => (
  <div className="animate-slide-in-left flex items-center justify-between p-4 rounded-lg bg-card/30 border border-border/50">
...
const FileUpload = ({ onFileSelect, onError }) => {
  ...
  return (
    <div className={`animate-scale-in relative border-2 border-dashed rounded-lg p-8 text-center transition-all duration-300 ${
        isDragOver 
